...
LaunchDecision LaunchController::decideLaunchMode()
{
    ...
    switch (state) {
        case AccountState::Errored:
            reauthReason = tr("An error occurred while refreshing '%1'").arg(m_accountToUse->profileName());
            break;
        case AccountState::Expired:
            reauthReason = tr("'%1' has expired and needs to be reauthenticated").arg(m_accountToUse->profileName());
            break;
        case AccountState::Disabled:
            reauthReason = tr("The launcher's client identification has changed");
            break;
        case AccountState::Gone:
            reauthReason = tr("'%1' no longer exists on the servers").arg(m_accountToUse->profileName());
            break;
        default:
            m_actualLaunchMode =
    ...
    if (reauthenticateAccount(m_accountToUse, reauthReason)) {

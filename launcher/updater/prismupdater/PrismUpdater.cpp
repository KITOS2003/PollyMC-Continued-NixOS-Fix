...
/** Strip a leading "v"/"V" from GitHub release tags so Version comparison stays numeric */
QString normalizeVersionTag(const QString& tag)
{
    if (tag.size() > 1 && (tag[0] == QLatin1Char('v') || tag[0] == QLatin1Char('V')) && tag[1].isDigit())
        return tag.mid(1);
    return tag;
}
...
PrismUpdaterApp::PrismUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for PollyMC-Continued"));
    ...
    if (!user_version.isEmpty()) {
        m_userSelectedVersion = Version(normalizeVersionTag(user_version));
    }
...
void PrismUpdaterApp::run()
{
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of PollyMC-Continued."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void PrismUpdaterApp::backupAppDir()
{
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "PollyMC-Continued", "bin", "share", "lib" });
...
int PrismUpdaterApp::parseReleasePage(const QByteArray* response)
{
    ...
    try {
        ...
        for (auto release_json : release_list) {
            ...
            release.version = Version(normalizeVersionTag(release.tag_name));
